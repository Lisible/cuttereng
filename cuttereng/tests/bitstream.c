...
void t_bitstream_init(void) {
  ...
  ASSERT_EQ(bitstream.current_byte_index, 0);
  ASSERT_EQ(bitstream.current_bit_offset, 0);
...
void t_bitstream_read_first_bits(void) {
  ...
  ASSERT_EQ(three_first_bits, 0b011);
...
void t_bitstream_read_bits_spanning_over_two_bytes(void) {
  ...
  ASSERT_EQ(next_8_bits, 0b10100100);
  u8 next_7_bits = bitstream_next_bits(&bitstream, 7);
  ASSERT_EQ(next_7_bits, 0b0111110);
}
